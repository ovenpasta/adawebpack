...
--  wasm/emcc fork of Ada.Environment_Variables.
--
--  The only change from the upstream body is in Value: upstream copies the
--  raw environment bytes with System.CRTL.strncpy.  System.CRTL declares
--  strncpy (and memcpy) as void-returning procedures, but emscripten's libc
--  defines them returning char*/void* (an i32).  WebAssembly enforces exact
--  function-type matching at link time, so wasm-ld cannot reconcile the
--  void-vs-pointer signatures and binds the explicit call to an `unreachable`
--  stub; a non-empty Value therefore traps at run time.
--
--  This fork sidesteps the C import entirely: it overlays an Ada String on
--  the returned pointer and copies via a plain array assignment, which the
--  code generator lowers through the llvm.memcpy intrinsic (correct
--  signature), so no libc symbol is referenced.
...
package body Ada.Environment_Variables is
   ...
   begin
      ...
      if Env_Value_Length > 0 then
         declare
            --  Overlay an Ada String on the C buffer and copy with a plain
            --  array assignment (lowered via the llvm.memcpy intrinsic), so
            --  no void-typed libc strncpy/memcpy symbol is referenced.
            Source : String (1 .. Env_Value_Length);
            for Source'Address use Env_Value_Ptr;
