...
--  WASM variant: identical public API to the upstream ghost spec, but the
--  generic packages provide trivial non-imported bodies so that their
--  instantiation in s-imagei.adb / s-imageu.adb doesn't trigger an ICE
--  (Assert_Failure sem_ch12.adb:18668) in the GNAT-LLVM compiler.
--
--  All entities remain Ghost and are erased at runtime, so the functional
--  behaviour is identical to the upstream version.
