...
is
   ...
   --  WASM-emcc variant: declare memcpy/memset as Address-returning
   --  functions to match s-memcop / s-memset's exports, otherwise
   --  wasm-ld inserts an unreachable trampoline for the signature
   --  mismatch and Get_Chunk traps on first use.
   function memcpy (s1, s2 : chars; n : size_t) return chars;
   pragma Import (C, memcpy);

   function memchr (s : chars; ch : int; n : size_t) return chars;
   pragma Import (C, memchr);

   function memset (b : chars; ch : int; n : size_t) return chars;
   pragma Import (C, memset);

   procedure Do_Memcpy (s1, s2 : chars; n : size_t);
   pragma Inline (Do_Memcpy);
   procedure Do_Memset (b : chars; ch : int; n : size_t);
   pragma Inline (Do_Memset);

   procedure Do_Memcpy (s1, s2 : chars; n : size_t) is
      Discard : chars := memcpy (s1, s2, n);
   begin
      null;
   end Do_Memcpy;

   procedure Do_Memset (b : chars; ch : int; n : size_t) is
      Discard : chars := memset (b, ch, n);
   begin
      null;
   end Do_Memset;
